...
%%% talk about the results instead of this
...
\subsection{Methods}
We used the Brown NLTK corpus for the model, along with the tags that come with it on NLTK. There are 1161192 words in the corpus, from a vocabulary of 56057 words.%%% number of bigrams, number of trigrams, vocabulary
...
In order to judge mellifluousness, we evaluated the generated text with human evaluators from Mechanical Turk.
...
%%% x: degree y: count %%%%%%%%%%%%%% CDF FUCKER FUCKER %%5 maybe not that one
...
There was only one person involved in the project. However, I have to note that much of the infrastructure of the Perceptron tagger was mutated from code by Matthew Honnibal. %cite that fucker
