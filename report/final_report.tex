...
\subsection{Methods}
We used the Brown NLTK corpus for the model, along with the tags that come with it on NLTK. There are 1161192 words in the corpus, in 57340 sentences, from a vocabulary of 56057 words, and 455267 unique bigrams. A train-test split was done of $\frac{2}{3}$ of the sentences being training examples and $\frac{1}{3}$ of the sentences being test examples.

%%%% put some real data statistics on here: number of words, vocabulary, number of sentences, bigrams. note that this also is the number of nodes and edges, the V and E.
%%% sample of Brown text
%%% cite the damn corpus

In order to judge mellifluousness, we evaluated the generated text with human evaluators from Mechanical Turk. %%% how?

It is not the goal of this project to build a state-of-the-art POS tagger. Rather, we want to see the usefulness of features of the generated stochastic Kronecker graph corresponding to the bigram graph of the POS corpus, to show that the structure of the graph can be harnessed to give information about the original corpus. %%% clarify

% we did a bigram baseline instead of trigram because of the argument from trajectories and stuff
% talk about the logic of the tries at features of the POS tag that we did
...
%%% mention the thoughts we had: we want to be able to compare things without the third-order thoughts. we don't want a state-of-the-art thing.
%%% mention stealing from Honiger
%%% explain the averaged perceptron
%%% explain the way we generated the text
...
%%% citations to add: snap, networkx, honiger, metzenmacher, RTG graph, some respected paper with averaged perceptron
